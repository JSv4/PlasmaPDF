...
class TestPackageJobResults(unittest.TestCase):
    def setUp(self) -> None:
        self.job_results: Dict[int, OpenContractsDocAnnotations] = {
            1: {
                "doc_labels": ["CONTRACT"],
                "labelled_text": [
                    {
        ...
        self.possible_span_labels: List[AnnotationLabelPythonType] = [
            {
                "id": "PARTY",
                "color": "#FF0000",
        ...
        self.possible_doc_labels: List[AnnotationLabelPythonType] = [
            {
                "id": "CONTRACT",
                "color": "#00FF00",
        ...
        self.possible_relationship_labels: List[AnnotationLabelPythonType] = []

        self.suggested_label_set: OpenContractsLabelSetType = {
            "id": "test-label-set",
    ...
    def test_package_job_results_successful(self) -> None:
        result = package_job_results_to_oc_generated_corpus_type(
            self.job_results,  # type: ignore
    ...
    def test_package_job_results_empty_input(self) -> None:
        empty_job_results: Dict[Union[int, str], OpenContractsDocAnnotations] = {}
        result = package_job_results_to_oc_generated_corpus_type(
            empty_job_results,
            self.possible_span_labels,
            self.possible_doc_labels,
    ...
    def test_package_job_results_invalid_input(self) -> None:
        ...
        with self.assertRaises(ValueError):
            package_job_results_to_oc_generated_corpus_type(
                invalid_job_results,  # type: ignore
                self.possible_span_labels,
                self.possible_doc_labels,
                self.suggested_label_set,
            )

    def test_package_job_results_multiple_docs(self) -> None:
        ...
        result = package_job_results_to_oc_generated_corpus_type(
            multi_doc_job_results,  # type: ignore
            self.possible_span_labels,
            self.possible_doc_labels,
            self.suggested_label_set,
        )
